...
- 任意の自己改善モード。プロジェクト専用の Hermes プロファイルとメモリを用意し、生きている Codex セッションから学習する。本線にはターンを注入しない。
...
- Gateway 起動中は管理UIの「Bot」で公開モデルごとのBotを複数作れる。固定指示、永続セッション、折り畳み、ストリーミング、停止、コピーに対応。
...
3. 新しい Codex セッションを開始し、`provider/model` を使う。
   Gateway 起動中は「Bot」から同じモデルへ直接質問できます。
4. 必要ならプロジェクトを追加し、同期プランを確認して、`.agents/plugins` のリポジトリローカルプラグインを入れる。

経路を使う間は CODETAS を起動したままにしてください。CODETAS が落ちた場合は公式 Codex へ一時退避し、復帰したらローカルゲートウェイへ戻します。カタログ変更後は Codex を完全終了して開き直し、モデル一覧を再読み込みします。

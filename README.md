...
CODETAS は、Codex のための実験的なローカル完結型コンパニオンです。プロジェクト
コンテキストブリッジ、Codex プラグイン、任意のローカルプロバイダーゲートウェイ
という3つの協調する面を持ちます。Hermes のプロジェクトコンテキストを検出し、
再利用できる内容を提示して、スキル・ライフサイクルフック・MCP サーバーからなる
レビュー可能な Codex 統合を提供します。
...
プロジェクト単位の統合を行う Codex プラグインです。ゲートウェイはデスクトップに
組み込むことも、スタンドアロンで動かすこともできます。プラグイン単体でも
使えます。

...
- CODETAS Desktop が生きている Codex セッションの jsonl を監視し、裏の学習 Agent が memory / skill を書く。本線は止まらない。セッションが終われば学習 Agent も終わる\n\
- 学習 Agent のきっかけ: ユーザーターン {memory_nudge} 回ごと、{flush} ターン目のチェックポイント、観測ツール {skill_nudge} またはユーザーターン {skill_nudge}、セッション終了時の flush\n\
- Gateway が動いているときだけ学習 Agent は起動する。プラグインの Stop continuation は sidecar がいないときの補助。明確な永続事実は nudge を待たず保存してよい\n\
\n\
ツール:\n\
...
\n\
bundled / hub / 外部所有スキルは編集しない。秘密、資格情報、会話全文、一時失敗、未検証のデッドエンドは残さない。明示的な利用者指示、AGENTS.md、より優先度の高い指示が衝突したらそちらを勝たせる。Desktop が起動し Gateway が動いているとき、裏の学習 Agent が jsonl から書く。プラグインは凍結注入のみ。\n",
...
mod tests {
    ...
    fn agent_instructions_include_soul_memory_and_self_learning() {
        ...
        assert!(instructions.contains("CODETAS-LEARNING-ORIGIN\nCODETAS-LEARNING-PROFILE:named:scyther"));
        let converted_inner = render_codex_agent("scyther", "検査担当", &instructions);
        let marker = "developer_instructions = \"\"\"\n";
        let start = converted_inner.find(marker).expect("developer_instructions");
        let inner = &converted_inner[start + marker.len()..];
        let end = inner.rfind("\n\"\"\"\n").expect("closed instructions");
        let developer_instructions = &inner[..end];
        assert!(developer_instructions.contains("CODETAS-LEARNING-ORIGIN\nCODETAS-LEARNING-PROFILE:named:scyther"));
        assert!(!developer_instructions.contains("# Generated by CODETAS"));
...
fn main() {
    ...
    let app = builder
        ...
        .manage(learning_runtime::LearningSupervisor::default())
        .setup(move |app| {

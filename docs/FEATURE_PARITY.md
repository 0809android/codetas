...
| --- | --- |
| Project sync | Read-only Hermes discovery, reviewable project-scoped Codex plan, and Codex profile conversion that embeds SOUL plus a fail-closed learning-loop contract (frozen snapshot, scopeToken writes, non-blocking in-turn reviews) |
